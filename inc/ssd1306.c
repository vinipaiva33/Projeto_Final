...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z')
  {
    index = (c - 'A' + 11) * 8; // Para letras maiúsculas
  }
  else if (c >= '0' && c <= '9')
  {
    index = (c - '0' + 1) * 8; // Para números
  }
  else if (c >= 'a' && c <= 'z')
  {
    index = (c - 'a' + 37) * 8; // Para letras minúsculas
  }

  // Caracteres especiais
  else if (c == '*') // Para o caractere 'º' (usaremos * por não ter º)
  {
    index = 63 * 8;  // Índice para o caractere 'º'
  }
  else if (c == ':') // Para o caractere ':'
  {
    index = 64 * 8;  // Índice para o caractere ':'
  }
  else if (c == '-') // Para o caractere '-'
  {
    index = 65 * 8;  // Índice para o caractere '-'
  }
  else if (c == '.') // Para o caractere '-'
  {
    index = 66 * 8;  // Índice para o caractere '.'
  }
  else if (c == '!') // Para o caractere '-'
  {
    index = 67 * 8;  // Índice para o caractere '.'
  }
  else if (c == '(') // Para o caractere '('
  {
    index = 68 * 8;  // Índice para o caractere '(' representando sinal de alerta
  }
  else if (c == ')') // Para o caractere ')'
  {
    index = 69 * 8;  // Índice para o caractere ')' representando sinal de alerta
  }
  else if (c == '>') // Para o caractere '>'
  {
    index = 70 * 8;  // Índice para o caractere '>' representando uma ->
  }
  else if (c == '<') // Para o caractere '<'
  {
